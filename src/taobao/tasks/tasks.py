...
from taobao.dao.models import Trade,TradeRule,TradeExtraInfo,Item,ProductRuleField
from taobao.picking.send_sms import send_message_to_buyer
...
def getTradePickingData():
    session = get_session()
    send_trades  = session.query(Trade)\
            .filter_by(status='WAIT_SELLER_SEND_GOODS')
    
    global_rules = session.query(TradeRule)\
            .filter_by(scope='trade',status='US')
    
    picking_data_list = []
    for trade in send_trades:
        trade_data = {}
        can_print  = True  #该交易能否打印
        dt         = datetime.datetime.now() 
        trade_extra_info = get_or_create_model(session,TradeExtraInfo,tid=trade.id)
                 
        if not trade_extra_info.is_picking_print:
            ...
            gather_memo = []
            seller_memo = json.loads(trade_extra_info.seller_memo or '{}')
            
            item_data = seller_memo.get('data',None) 
