...
class ScanWeightPanel(wx.Panel):
    ...
    def onOutsidTextChange(self,evt):
        ...
        if not out_sid:
            return 
        try:
            trade  = api.getWeightTradeInfo(out_sid)
    ...
    def setTradeInfoPanel(self,trade):
        ...
        self.order_content3.SetValue(trade['trade_type'])
        self.order_content4.SetValue(trade['buyer_nick'])
        self.order_content5.SetValue(trade['company_name'])
    ...
    def save_weight_to_trade(self,trade,weight):
        ...
        api.saveWeight2Trade(trade['package_no'], weight)
        trade['weight'] = weight
