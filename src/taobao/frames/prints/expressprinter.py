...
from taobao.dao.models import PackageOrder, PackageSkuItem, Item, ClassifyZone
...
class ExpressPrinter(wx.Frame):
    ...
    def createHtml(self, trade_ids=[]):
        ...
        try:
            ...
            html = u'<html><head></head><body style="text-align:center;">对不起，你还没有添加%s的物流单模板。</body></html>' % trades[0][
                'company_name']
        return html
    ...
    def onPreview(self, event):
        ...
        with create_session(self.Parent) as session:
            trades = session.query(PackageOrder).filter(PackageOrder.pid.in_(self.trade_ids)).filter_by(
        ...
        with create_session(self.Parent) as session:
            ...
            for trade in send_trades:
                ...
                if trade_data['company_code'].upper() in YUNDA_CODE:
                    ...
                    trade_data['zone'] = zone and '%s %s' % (zone['name'], zone['code']) or ''
